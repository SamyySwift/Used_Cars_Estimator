...
st.write("This Streamlit application estimates the price of used cars using machine learning models. Users can select between a Decision Tree and Gradient Boosting model to make predictions based on various car features.")


# --------------- Slider Section-----------------------------
st.sidebar.markdown("## :blue[Select Model]")

with st.sidebar:
    selected_model = pills("Trained Models", ["Decision Tree", "Gradient Boosting"])
st.markdown("#")
...
model_name_map = {'Renegade': 829, 'Discovery Sport': 319, 'WRX STI': 1062, 'Discovery': 317, 'Range Rover Velar': 818, 'MAZDA3': 609, 'Range Rover Evoque': 814, 
              ...
              '4 Series': 35, 'GLE-Class': 1600, 'Escalade': 369, 'Tucson': 1020, '2 Series': 6, 'Pacifica': 709, 'Suburban': 963, 'Camaro': 223, 'Cruze': 293, 'Trax': 1016, 'SQ5': 875, 
              ...
              'LS 1600': 570, 'X4': 1072, 'X1': 1068, 'Accord': 114, 'Civic Coupe': 250, 'CLS-Class': 197, 'G35': 448, 'Passat': 717, 'Sprinter': 954, 'Q5': 744, 'Canyon': 228, 
            ...
            'LS 500': 571, 'F-TYPE': 406, 'LC': 561, 'Mulsanne': 668, 'xD': 1124, 'Endeavor': 358, 'Quest': 763, 'iA': 1118, 'H3': 511, 'Amanti': 126, 'Envoy XL': 362, 'Caliber': 220,
                ...
                'Insight': 537, 'S8': 865, 'C8': 189, 'Dawn': 310, 'Phantom Drophead Coupe': 724, 'SLR McLaren': 873, '250 GT': 12, 'Phantom': 723, 'Urus': 1027, 'Flying Spur': 428,
                    ...
                    'MGB': 617, '300-Class': 19, 'RC 200t': 783, 'GS 1600': 470, 'Regal TourX': 825, 'Le Baron': 587, 'Classic': 256, 'Aurora': 140, 'fortwo electric drive': 1114, 'RAM 4500 Chassis ': 776, 'Ranchero': 812, 'Satellite': 891, 'Bentayga Hybrid': 161, 'LFA': 564, 
...
st.markdown("#")
st.subheader(":blue[Select Car Features]")
...
with col2:
    ...
    mileage = col2.number_input("Mileage (MPG)", min_value=0.0, max_value=4290461.0, step=1.0)
    horsepower = col2.slider("Horsepower (HP)", 70.0, 903.0, 1.0)
    fuel_tank_volume = col2.slider("Fuel Tank Volume (Gallons)", 1.9, 64.0, 1.0)
    height = col2.slider("Height (Inches)", 0, 603, 1)
    width = col2.slider("Width (Inches)", 42.5, 109.0, 1.0)
...
with col3:
    ...
    daysonmarket = col3.slider("Days On Market (Days)", 0, 2716, 1)
    wheelbase = col3.slider("Wheel Base (Inches)", 73.5, 201.0, 1.0)
    power = col3.slider("Power (HP)", 70.0, 903.0, 1.0)
    engine_displacement = col3.slider("Engine Displacement (CC)", 700.0, 8400.0, 1.0)
...
try:
    if predict and selected_model == "Decision Tree":
        ...
        price_naira = predicted_price[0] * 1600  # Assuming 1 USD = 1600 NGN
        st.markdown("#")
        st.subheader("**:blue[Model's Prediction]**")
        st.markdown(
                f"**The :red[{selected_model}] model estimates that the car is worth :red[💲{round(predicted_price[0], 2)}] (₦{round(price_naira, 2)})**"
            )
        ...
        price_naira = predicted_price[0] * 1600  # Assuming 1 USD = 1600 NGN
        st.markdown(
            f"**The :red{selected_model} model estimates that the car is worth :red[💲{round(predicted_price[0], 2)}] equivalent to (₦{round(price_naira, 2)})**"
            )  
    else:
        st.empty()
